...
For video editing (add, remove, restyle, re-light), use the Runway Aleph 2 Edit template instead.

## Model Specs

| Spec | Gen-4 Turbo | Gen-4.5 |
|------|-------------|---------|
| API model ID | `gen4_turbo` | `gen4.5` |
...
Gen-4.5 allows more precise camera control described separately from subject movement, so camera choreography and subject action can be specified independently within one prompt.
